...
// fakeEncryptor is an in-process Encryptor that wraps the DEK with a fixed
// AES-GCM master key instead of round-tripping through KMS.
...
func TestEncryptor_DistinctCiphertextsForSamePlaintext(t *testing.T) {
	// Fresh DEK + nonce per call must produce distinct ciphertexts.
	e := newFakeEncryptor(t)
	ctx := context.Background()
