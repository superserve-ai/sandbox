// Package secrets implements envelope encryption: AES-256-GCM with a per-row
// data key (DEK) wrapped by a Cloud KMS key encryption key (KEK).
package secrets
...
type Encrypted struct {
	...
	Ciphertext []byte
	// EncryptedDEK is the per-row data key wrapped by KEKID.
	EncryptedDEK []byte
...
// NewKMSEncryptor returns an Encryptor that wraps DEKs with the given KMS key resource name.
...
// Encrypt seals plaintext with a fresh DEK and wraps the DEK via KMS.
...
// Decrypt unwraps the DEK using enc.KEKID and opens the ciphertext.
