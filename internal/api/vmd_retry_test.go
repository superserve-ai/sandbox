...
func TestRetryBootOnDeadline(t *testing.T) {
	...
	boot := func(results []error, calls *int) func(context.Context) (string, uint32, uint32, string, error) {
		return func(context.Context) (string, uint32, uint32, string, error) {
			err := results[*calls]
			*calls++
			if err != nil {
				return "", 0, 0, "", err
	...
	t.Run("success first try", func(t *testing.T) {
		calls := 0
		ip, vcpu, mem, _, retried, err := retryTransientBoot(context.Background(), "sb", boot([]error{nil}, &calls))
	...
	t.Run("non-deadline error is not retried", func(t *testing.T) {
		calls := 0
		_, _, _, _, retried, err := retryTransientBoot(context.Background(), "sb", boot([]error{other}, &calls))
	...
	t.Run("grpc deadline retried once then succeeds with attempt-2 outputs", func(t *testing.T) {
		calls := 0
		ip, _, _, _, retried, err := retryTransientBoot(context.Background(), "sb", boot([]error{grpcDeadline, nil}, &calls))
	...
	t.Run("wrapped context deadline from the interceptor is retried", func(t *testing.T) {
		calls := 0
		_, _, _, _, retried, err := retryTransientBoot(context.Background(), "sb", boot([]error{wrappedCtxDeadline, nil}, &calls))
	...
	t.Run("unavailable that outlived the interceptor window is retried", func(t *testing.T) {
		calls := 0
		_, _, _, _, retried, err := retryTransientBoot(context.Background(), "sb", boot([]error{status.Error(codes.Unavailable, "connection refused"), nil}, &calls))
		if err != nil || !retried || calls != 2 {
			t.Fatalf("err=%v retried=%v calls=%d, want nil/true/2", err, retried, calls)
		}
	})
	...
	t.Run("deadline twice returns the second error", func(t *testing.T) {
		calls := 0
		_, _, _, _, retried, err := retryTransientBoot(context.Background(), "sb", boot([]error{grpcDeadline, grpcDeadline}, &calls))
	...
	t.Run("dead caller skips the retry", func(t *testing.T) {
		...
		cancel()
		calls := 0
		_, _, _, _, retried, err := retryTransientBoot(parent, "sb", boot([]error{grpcDeadline, nil}, &calls))
