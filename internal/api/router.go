...
func SetupRouter(ctx context.Context, h *Handlers, pool *pgxpool.Pool) *gin.Engine {
	...
	api.Use(APIKeyAuth(pool), TeamRateLimit(ctx, DefaultTeamRateLimitConfig()))
	{
		...
		api.POST("/sandboxes/:sandbox_id/checkpoints", h.CreateCheckpoint)
		api.GET("/sandboxes/:sandbox_id/checkpoints", h.ListCheckpoints)
		api.POST("/sandboxes/:sandbox_id/branches", h.CreateBranch)
		api.POST("/sandboxes/:sandbox_id/rollback", h.RollbackState)
