...
func TestIntegration_CreateSandbox_AllowInternetAccessFalse(t *testing.T) {
	teamID, apiKey := seedTeamAndKey(t)
	r := newRouter(t)

	cw := do(r, "POST", "/sandboxes", apiKey,
		`{"name":"no-net","allow_internet_access":false}`)
	if cw.Code != http.StatusCreated {
		t.Fatalf("create: %d %s", cw.Code, cw.Body.String())
	}
	sid := mustJSON(t, cw)["id"].(string)
	...
	// Persisted network_config should contain the injected deny rule.
	raw, err := testQueries.GetSandboxNetworkConfig(context.Background(), db.GetSandboxNetworkConfigParams{
		ID:     sandboxUUID,
		TeamID: teamID,
	})
	if err != nil {
	...
	if raw == nil || !strings.Contains(string(raw), "0.0.0.0/0") {
		t.Errorf("expected persisted deny rule for 0.0.0.0/0, got: %s", string(raw))
	}
}

func TestIntegration_CreateSandbox_AllowInternetAccessTrue(t *testing.T) {
	teamID, apiKey := seedTeamAndKey(t)
	r := newRouter(t)

	cw := do(r, "POST", "/sandboxes", apiKey,
		`{"name":"open-net","allow_internet_access":true}`)
	if cw.Code != http.StatusCreated {
		t.Fatalf("create: %d %s", cw.Code, cw.Body.String())
	}
	sid := mustJSON(t, cw)["id"].(string)
	...
	// No network config should be persisted — allow_internet_access: true
	// is the default and does not inject any rules.
	raw, err := testQueries.GetSandboxNetworkConfig(context.Background(), db.GetSandboxNetworkConfigParams{
		ID:     sandboxUUID,
		TeamID: teamID,
	})
	...
	if raw != nil && len(raw) > 0 {
		t.Errorf("expected no persisted network_config, got: %s", string(raw))
	}
}

func TestIntegration_CreateSandbox_ExplicitNetworkWinsOverAllowFlag(t *testing.T) {
	teamID, apiKey := seedTeamAndKey(t)
	r := newRouter(t)

	// allow_internet_access: false would inject a broad deny, but we
	// also pass an explicit network config — the explicit config should
	// win (deny only api.anthropic.com, not everything).
	cw := do(r, "POST", "/sandboxes", apiKey,
		`{"name":"conflict","allow_internet_access":false,"network":{"deny_out":["1.2.3.4/32"]}}`)
	if cw.Code != http.StatusCreated {
		t.Fatalf("create: %d %s", cw.Code, cw.Body.String())
	}
	sid := mustJSON(t, cw)["id"].(string)
	...
	// User's 1.2.3.4/32 should be preserved; 0.0.0.0/0 should NOT be injected.
	if !strings.Contains(string(raw), "1.2.3.4/32") {
		t.Errorf("expected user-supplied 1.2.3.4/32 to be persisted, got: %s", string(raw))
	}
	if strings.Contains(string(raw), "0.0.0.0/0") {
		t.Errorf("allow_internet_access: false should NOT have injected 0.0.0.0/0 when user provided deny_out, got: %s", string(raw))
	}
}
