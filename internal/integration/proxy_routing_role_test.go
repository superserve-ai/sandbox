...
func TestIntegration_ProxyRoutingRoleMigration(t *testing.T) {
	...
	migration, err := os.ReadFile("../../supabase/migrations/20260923000010_sandbox_proxy_router.sql")
	if err != nil {
		t.Fatal(err)
	}
 ...
 GRANT SELECT ON public.example_routing_grant TO sandbox_proxy_router;`)
	for _, state := range []string{"existing", "stale", "partial", "table_reads", "column_reads", "role_attributes"} {
		t.Run(state, func(t *testing.T) {
			...
			if state == "table_reads" {
				exec(t, `GRANT SELECT ON public.sandbox, public.host TO sandbox_proxy_router`)
			}
			if state == "column_reads" {
				exec(t, `GRANT SELECT (secret_env_fingerprint) ON public.sandbox TO sandbox_proxy_router;
                    GRANT SELECT (region) ON public.host TO sandbox_proxy_router;`)
			}
			if state == "stale" {
    ...
    AND rolconnlimit = $1 AND rolconfig @> ARRAY['default_transaction_read_only=on','statement_timeout=500ms']
    FROM pg_roles WHERE rolname = 'sandbox_proxy_router'`, wantLimit).Scan(&valid)
			if err != nil || !valid {
