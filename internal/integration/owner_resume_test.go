...
func TestIntegration_OwnerResumeCapabilities(t *testing.T) {
	...
	for _, state := range []string{"active", "draining", "provisioning", "unhealthy"} {
		for _, freshness := range []string{"current", "missing", "stale", "one-stale", "null-heartbeat", "expired", "at-cutoff", "fresh-near-cutoff"} {
			t.Run(state+"/"+freshness, func(t *testing.T) {
				...
				locked, err := testQueries.LockedHostHasCapabilities(ctx, db.HostHasCapabilitiesParams{AllowedStatuses: []string{"active", "draining"}, HostID: hostID, RequiredCapabilities: required, HeartbeatAfter: cutoff})
				...
				active, err := testQueries.LockedHostHasCapabilities(ctx, db.HostHasCapabilitiesParams{AllowedStatuses: []string{"active"}, HostID: hostID, RequiredCapabilities: required})
	...
	missing, err := testQueries.LockedHostHasCapabilities(ctx, db.HostHasCapabilitiesParams{AllowedStatuses: []string{"active", "draining"}, HostID: "missing-owner", RequiredCapabilities: required, HeartbeatAfter: pgtype.Timestamptz{Time: time.Now().Add(-2 * time.Minute), Valid: true}})
	if err != nil || missing {
