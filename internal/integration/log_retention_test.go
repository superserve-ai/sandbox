...
func TestIntegration_LogRetentionMaintainsDayPartitions(t *testing.T) {
	...
	var days int
	if err := testPool.QueryRow(ctx, `SELECT count(*) FROM pg_inherits WHERE inhparent = 'net_flow'::regclass`).Scan(&days); err != nil || days < 15 {
		t.Fatalf("net_flow partitions = %d, err = %v; want 7 days back, today, and 7 ahead", days, err)
	}
	if _, err := testPool.Exec(ctx, `INSERT INTO net_flow (team_id, sandbox_id, protocol, dst_ip, dst_port, verdict) VALUES ($1, $2, 'tls', '1.1.1.1', 443, 'allowed')`, uuid.New(), uuid.New()); err != nil {
		t.Fatalf("insert today: %v", err)
	}
	if _, err := testPool.Exec(ctx, `INSERT INTO net_flow (ts, team_id, sandbox_id, protocol, dst_ip, dst_port, verdict) VALUES (now() - interval '9 days', $1, $2, 'tls', '1.1.1.1', 443, 'allowed')`, uuid.New(), uuid.New()); err == nil {
