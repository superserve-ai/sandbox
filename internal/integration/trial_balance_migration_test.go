...
func TestTrialBalanceMigrationUpgradeConverges(t *testing.T) {
	...
	cleanMigration, err := os.ReadFile("../../supabase/migrations/20260917000008_billing_trial_balance.sql")
	if err != nil {
		t.Fatal(err)
	}
	...
	migration, err := os.ReadFile("../../supabase/migrations/20260917000005_trial_balance_corrections.sql")
	if err != nil {
		t.Fatal(err)
	}
...
func TestTrialBalanceUnconsumedGrantRemainsEligible(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "unused grant"},
		{name: "newly opened interval", interval: true, newly: true},
		{name: "zero rates", interval: true, zeroRate: true},
		{name: "missing rates", interval: true, noRates: true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			ctx := context.Background()
			teamID := mustCreateTeam(t, ctx, "trial-unconsumed-"+uuid.NewString()[:8])
			sandboxID := seedPrivatePreviewSandbox(t, teamID, testDefaultHostID, "trial-unconsumed")
			tx, err := testPool.Begin(ctx)
			if err != nil {
				t.Fatal(err)
			}
			...
			if _, err := tx.Exec(ctx, `INSERT INTO team_credit_grant
				...
				VALUES ($1, 5, 5, 'signup trial credit', now() - interval '1 day', now() + interval '1 day')`, teamID); err != nil {
				t.Fatal(err)
			}
			planKey := "trial-unconsumed-" + uuid.NewString()
			...
			if !tc.noRates {
				if _, err := tx.Exec(ctx, `INSERT INTO pricing_rate (plan_key, resource, unit, price_usd, effective_from)
					VALUES ($1, 'vcpu', 'second', CASE WHEN $2 THEN 0 ELSE 1 END, now() - interval '1 day')`, planKey, tc.zeroRate); err != nil {
					t.Fatal(err)
				}
			}
			...
			if tc.interval {
				// Transaction time keeps the newly opened interval at zero elapsed time.
				if _, err := tx.Exec(ctx, `INSERT INTO sandbox_compute_billing_interval
					(sandbox_id, team_id, vcpu_count, memory_mib, started_at)
					VALUES ($1, $2, 1, 1024, CASE WHEN $3 THEN now() ELSE now() - interval '1 hour' END)`, sandboxID, teamID, tc.newly); err != nil {
					t.Fatal(err)
				}
			}
			q := db.New(tx)
			...
			if balance.State != "active" || !balance.Eligible {
				t.Fatalf("unconsumed trial: state=%q eligible=%t", balance.State, balance.Eligible)
			}
			for _, value := range []struct {
				...
				{"grant", balance.GrantUsd, 5},
				{"consumed", balance.ConsumedUsd, 0},
				{"remaining", balance.RemainingUsd, 5},
			} {
			...
			var cached bool
			if err := tx.QueryRow(ctx, `SELECT eligible FROM team_trial_eligibility_cache WHERE team_id = $1`, teamID).Scan(&cached); err != nil {
				t.Fatal(err)
			}
			if !cached {
				t.Fatal("unconsumed trial must remain eligible in the enforcement cache")
			}
			eligible, err := q.IsTeamSandboxBillingEligible(ctx, teamID)
			if err != nil || !eligible {
				t.Fatalf("enforcement eligible=%t, error=%v, want true", eligible, err)
			}
		})
	}
}
