...
SELECT (compute.amount + CASE WHEN feature_enabled('billing_storage_billing_enabled', $1) THEN COALESCE((SELECT SUM(EXTRACT(EPOCH FROM (COALESCE(s.ended_at, now()) - s.started_at)) * s.disk_mib / 1024.0 * rates.storage) FROM recent_storage s CROSS JOIN rates), 0) ELSE 0 END)::numeric AS spent_usd,
