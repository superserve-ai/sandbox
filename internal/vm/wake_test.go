...
func TestWakeStateSurvivesRecordRoundTrip(t *testing.T) {
	frozen := true
	rec := toRecord(&VMInstance{ID: "vm", WakePending: true, ClockFrozen: true, SnapshotWorkloadFrozen: &frozen, CorrectsWallClock: &frozen, FreezeToken: "tok", WakeToken: "tok", ArtifactID: "a"})
...
func TestResumeWakesFrozenWorkloadBeforeCommit(t *testing.T) {
	...
	inst := &VMInstance{
		...
		SnapshotWorkloadFrozen: &frozen, CorrectsWallClock: &frozen, FreezeToken: "tok",
...
func TestFrozenResumeFailureRevertsToPaused(t *testing.T) {
	...
	inst := &VMInstance{
		ID: "vm-1", Status: StatusPaused, Supervision: SupervisionUnit,
		SnapshotPath: snapPath, MemFilePath: memPath, DiskPath: rootfs,
		SnapshotWorkloadFrozen: &frozen, CorrectsWallClock: &frozen, PausedAt: pausedAt, FreezeToken: "rec",
...
func TestFrozenResumeRollbackThatCannotPersistKeepsTheSlot(t *testing.T) {
	...
	inst := &VMInstance{
		ID: "vm-1", Status: StatusPaused, Supervision: SupervisionUnit,
		SnapshotPath: snapPath, MemFilePath: memPath, DiskPath: rootfs,
		SnapshotWorkloadFrozen: &frozen, CorrectsWallClock: &frozen, FreezeToken: "rec",
	}
