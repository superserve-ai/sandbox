...
func TestReviveVMGuards(t *testing.T) {
	...
	if _, err := m.ReviveVM(context.Background(), "../escape", disk, "", false, 1, 128, nil); status.Code(err) != codes.InvalidArgument {
		t.Fatalf("traversal id: %v, want InvalidArgument", err)
	}
	if _, err := m.ReviveVM(context.Background(), "vm-1", filepath.Join(t.TempDir(), "absent.ext4"), "", false, 1, 128, nil); status.Code(err) != codes.InvalidArgument {
	...
	m.mu.Unlock()
	if _, err := m.ReviveVM(context.Background(), "vm-paused", disk, "", false, 1, 128, nil); status.Code(err) != codes.FailedPrecondition {
