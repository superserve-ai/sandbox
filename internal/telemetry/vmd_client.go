...
func (c *instrumentedVMDClient) ResumeInstance(ctx context.Context, instanceID, snapshotPath, memPath string, networkConfig []byte, previewAccess string, previewPorts map[int32]vmdclient.PortPolicy, previewPolicyRevision int64, backupAnchor map[string]string) (ipAddress string, actualVcpu, actualMemMiB uint32, attested vmdclient.ResumeAttestation, err error) {
	started := time.Now()
	defer func() { c.record(ctx, "ResumeVM", started, err) }()
	return c.next.ResumeInstance(ctx, instanceID, snapshotPath, memPath, networkConfig, previewAccess, previewPorts, previewPolicyRevision, backupAnchor)
}
