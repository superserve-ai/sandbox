...
func TestRoutingHandlerHTTPBodiesOverMTLS(t *testing.T) {
	...
	directRouter := httptest.NewServer(NewRoutingHandler([]string{"sandbox.test"}, "host-a", RouteLookupFunc(func(context.Context, string) (SandboxRoute, error) {
		return SandboxRoute{Generation: 42, HostID: "host-b", ProxyAddr: directAddr}, nil
...
func TestRoutingHandlerUnavailablePeerTargetReturns502(t *testing.T) {
	...
	router := httptest.NewServer(NewRoutingHandler([]string{"sandbox.test"}, "host-a", RouteLookupFunc(func(context.Context, string) (SandboxRoute, error) {
		return SandboxRoute{Generation: 42, HostID: "host-b", ProxyAddr: peerAddr}, nil
	}), routePeerFunc(func(ctx context.Context, host string, endpoint PeerEndpoint) (PeerStream, error) {
		opens.Add(1)
		return peers.OpenStream(ctx, host, endpoint)
