...
func (p *peerPool) OpenStream(ctx context.Context, host string, endpoint PeerEndpoint) (PeerStream, error) {
	if p.cfg.Dial == nil {
		return nil, errors.New("peer dialer is required")
	}
	if host == "" || endpoint.Address == "" || endpoint.Generation == 0 {
		return nil, errors.New("peer host, address, and positive endpoint generation required")
	}
	p.mu.Lock()
	if p.closed {
		p.mu.Unlock()
	...
	h := p.hosts[host]
	if h == nil {
		if latest, ok := p.endpoints[host]; ok {
			if endpoint.Generation == latest.Generation && endpoint.Address != latest.Address {
				p.mu.Unlock()
				return nil, errors.New("conflicting peer addresses for the same endpoint generation")
			}
			if endpoint.Generation < latest.Generation {
				endpoint = latest
			}
		}
		delete(p.endpoints, host)
		h = &hostPool{parent: p, host: host, addr: endpoint.Address, endpointGeneration: endpoint.Generation}
		p.hosts[host] = h
	}
	...
	h.mu.Lock()
	if endpoint.Generation == h.endpointGeneration && endpoint.Address != h.addr {
		h.mu.Unlock()
		p.mu.Unlock()
		return nil, errors.New("conflicting peer addresses for the same endpoint generation")
	}
	...
	endpointChanged := endpoint.Generation > h.endpointGeneration
	h.mu.Unlock()
	if endpointChanged {
		h.replaceLocked(endpoint.Address, endpoint.Generation)
	}
	p.mu.Unlock()
	defer func() {
		h.mu.Lock()
...
// pending observations per transport and one emitter, without delaying callers.
...
func (h *hostPool) replaceLocked(addr string, generation uint64) {
	h.mu.Lock()
	h.replacing = true
	h.addr = addr
	h.endpointGeneration = generation
	h.broadcastLocked()
	if h.dialCancel != nil {
		h.dialCancel()
	}
...
retry:
	for {
		...
		// Cleanup retains its budget until the physical closer returns.
		if h.dialing || usable >= h.parent.cfg.MaxConnections || len(h.conns)+h.retired+h.discarding >= 2*h.parent.cfg.MaxConnections {
			if err := h.waitCapacityLocked(ctx); err != nil {
				return nil, err
			}
			continue
		}
		...
		if h.closed {
			close(dialDone)
			h.dialDone = nil
			h.discardDialLocked(client, closer)
			h.notifyLocked()
			h.mu.Unlock()
			h.parent.observations.record(h.parent.cfg.Telemetry, peerObservation{duration: duration, result: result})
			return nil, errors.New("peer host closed")
		}
		...
		if e == nil {
			c := &peerConn{client: client, closer: closer, max: h.parent.cfg.StreamsPerConnection, closed: make(chan struct{}), streamMetrics: &h.parent.streamMetrics, connectionMetrics: &h.parent.connectionMetrics}
			...
			h.failures = 0
			h.retryAt = time.Time{}
			emitConnection = c.connectionMetrics.queue(1)
		} else {
		...
		h.notifyLocked()
		h.mu.Unlock()
		h.parent.observations.record(h.parent.cfg.Telemetry, peerObservation{duration: duration, result: result, reconnect: recovering && !peerCallerCanceled(e)})
		if emitConnection {
			go h.parent.connectionMetrics.flush(h.parent.cfg.Telemetry.PeerConnection)
		}
		if e != nil {
			h.evictIfEmpty()
			return nil, e
...
// Keep discarded dial results within the host budget until cleanup completes.
func (h *hostPool) discardDialLocked(client PeerClient, closer io.Closer) {
	h.discarding++
	go func() {
...
// Capacity notifications transfer to one waiter. Cancellation after selection
// passes the notification on so available capacity cannot become stranded.
func (h *hostPool) waitCapacityLocked(ctx context.Context) error {
	ready := make(chan struct{})
	e := h.waiters.PushBack(ready)
	h.mu.Unlock()
	select {
	case <-ready:
	case <-ctx.Done():
...
func (h *hostPool) wakeOneLocked() {
	if e := h.waiters.Front(); e != nil {
		h.waiters.Remove(e)
		close(e.Value.(chan struct{}))
	}
}

func (h *hostPool) broadcastLocked() {
	for h.waiters.Len() > 0 {
		h.wakeOneLocked()
	}
	h.notifyLocked()
...
func (h *hostPool) evictIfEmpty() {
	...
	h.mu.Lock()
	defer h.mu.Unlock()
	if h.closed || h.replacing || h.openers != 0 || h.retired != 0 || h.discarding != 0 || h.dialing {
		return
	}
	for _, c := range h.conns {
		c.mu.Lock()
	...
	if h.failures > 0 || len(h.conns) > 0 {
		if h.eviction != nil {
			h.eviction.Stop()
		}
		h.evictionGeneration++
		generation := h.evictionGeneration
		idle := h.parent.cfg.IdleTimeout
		if len(h.conns) == 0 {
			idle = peerReconnectMaxBackoff
		}
		h.eviction = time.AfterFunc(idle, func() { h.evictIdle(generation) })
		return
	}
...
// Requests can carry arbitrarily old discovery results, so endpoint metadata
// must outlive idle connection/retry state and remain until transport shutdown.
// The parent and host locks are held; the caller has checked host identity.
func (h *hostPool) evictLocked() {
	if !h.parent.closed {
		if h.parent.endpoints == nil {
			h.parent.endpoints = make(map[string]PeerEndpoint)
		}
		h.parent.endpoints[h.host] = PeerEndpoint{Address: h.addr, Generation: h.endpointGeneration}
	}
...
func (h *hostPool) evictIdle(generation uint64) {
	...
	if h.openers != 0 || h.dialing || h.retired != 0 || h.discarding != 0 || h.parent.hosts[h.host] != h {
		return
	}
	for _, c := range h.conns {
		c.mu.Lock()
...
func (h *hostPool) detachFailed(c *peerConn) func() {
	...
	if found {
		return func() {
			c.closeWithStreams()
			h.remove(c)
			h.parent.observations.record(h.parent.cfg.Telemetry, peerObservation{kind: "failure"})
		}
	}
	return nil
}
...
func (h *hostPool) scheduleDrain(c *peerConn) {
	...
	if idle {
		...
		go func() {
			c.close()
			h.remove(c)
			h.parent.observations.record(h.parent.cfg.Telemetry, peerObservation{kind: "drain", forced: false})
		}()
		return
	}
	go func() {
		...
		h.parent.observations.record(h.parent.cfg.Telemetry, peerObservation{kind: "drain", forced: active > 0})
	}()
}
func (h *hostPool) releaseOpen(c *peerConn) {
	...
	if reclaim {
		go func() {
			c.close()
			h.remove(c)
			h.parent.observations.record(h.parent.cfg.Telemetry, peerObservation{kind: "drain"})
		}()
	}
}
...
draining:
	...
	for _, c := range cs {
		...
		h.parent.observations.record(h.parent.cfg.Telemetry, peerObservation{kind: "drain", forced: true})
	}
}
...
// A known I/O result must not wait for an external closer. Repeated I/O errors
// share one cleanup worker even when the physical close remains blocked.
func (s *countedPeerStream) closeAsync() {
	s.finish()
	s.asyncClose.Do(func() { go s.Close() })
}

func (s *countedPeerStream) Close() error {
	s.finish()
	s.once.Do(func() { s.closeErr = s.PeerStream.Close() })
	return s.closeErr
}

// Cancellation and accounting complete independently of physical cleanup so a
// completed stream cannot retain capacity while its custom closer is blocked.
func (s *countedPeerStream) finish() {
	if s.cancel != nil {
		s.cancel()
	}
	s.accounted.Do(func() {
		...
		if reclaim {
			go func() {
				...
				s.host.parent.observations.record(s.tele, peerObservation{kind: "drain"})
			}()
		} else {
			s.host.evictIfEmpty()
		}
