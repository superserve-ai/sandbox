...
func TestValidateSandboxRouteNormalizesAndRejectsUnusableEndpoints(t *testing.T) {
	valid := SandboxRoute{Generation: 1, HostID: " host-a ", ProxyAddr: " 127.0.0.1:443 "}
	if err := ValidateSandboxRoute(valid); err != nil {
		t.Fatalf("trimmed usable endpoint rejected: %v", err)
	}
	for _, route := range []SandboxRoute{
		{Generation: 1, HostID: "host-a", ProxyAddr: "   "},
		{Generation: 1, HostID: "host-a", ProxyAddr: "127.0.0.1"},
		{Generation: 1, HostID: "host-a", ProxyAddr: "127.0.0.1:abc"},
		{Generation: 1, HostID: "host-a", ProxyAddr: "127.0.0.1:0"},
	} {
...
func TestNormalizeSandboxRouteTrimsValues(t *testing.T) {
	route, err := NormalizeSandboxRoute(SandboxRoute{Generation: 1, HostID: " host-a ", ProxyAddr: " 127.0.0.1:443 "})
	if err != nil {
		t.Fatal(err)
	}
