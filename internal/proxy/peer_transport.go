...
// PeerEndpoint couples an address with its authoritative host incarnation.
// Generation must be positive and increase whenever the address or incarnation
// changes; callers must preserve the generation from endpoint discovery.
...
type PeerTransport interface {
	OpenStream(context.Context, string, PeerEndpoint) (PeerStream, error)
	Close() error
}
