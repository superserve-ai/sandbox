...
-- name: ReferencedSharedBases :many
...
WHERE bg.bucket = sqlc.arg(bucket)::text
  ...
  AND (bg.purged_at IS NULL
       OR greatest(bg.reported_at, bg.purged_at) > now() - make_interval(secs => sqlc.arg(grace_seconds)::float8));

-- name: RecordedSharedBasePaths :many
