...
WITH input_order AS MATERIALIZED (
  -- Keep the generated sqlc call order stable for existing mocks and binaries.
  SELECT sqlc.arg(id)::uuid, sqlc.arg(team_id)::uuid, sqlc.arg(name)::text,
         sqlc.arg(status)::sandbox_status, sqlc.arg(vcpu_count)::int,
         sqlc.arg(memory_mib)::int, sqlc.arg(host_id)::text,
         sqlc.narg(ip_address)::inet, sqlc.narg(pid)::int,
         sqlc.narg(snapshot_id)::uuid, sqlc.narg(timeout_seconds)::int,
         sqlc.arg(metadata)::jsonb, sqlc.narg(template_id)::uuid,
         sqlc.narg(snapshot_path)::text, sqlc.narg(mem_path)::text,
         sqlc.narg(base_path)::text, sqlc.narg(delta_path)::text,
         sqlc.narg(auto_delete_seconds)::int,
         sqlc.arg(preview_access)::text
),
...
WITH input_order AS MATERIALIZED (
  -- The owner-team parameter is reused rather than duplicated; otherwise keep
  -- the legacy create argument order so SQL-mocking callers stay stable.
  SELECT sqlc.arg(id)::uuid, sqlc.arg(team_id)::uuid, sqlc.arg(name)::text,
         sqlc.arg(status)::sandbox_status, sqlc.arg(vcpu_count)::int,
         sqlc.arg(memory_mib)::int, sqlc.arg(host_id)::text,
         sqlc.narg(ip_address)::inet, sqlc.narg(pid)::int,
         sqlc.narg(snapshot_id)::uuid, sqlc.narg(timeout_seconds)::int,
         sqlc.arg(metadata)::jsonb, sqlc.arg(template_id)::uuid,
         sqlc.arg(system_team_id)::uuid, sqlc.narg(snapshot_path)::text,
         sqlc.narg(mem_path)::text, sqlc.narg(base_path)::text,
         sqlc.narg(delta_path)::text, sqlc.narg(auto_delete_seconds)::int,
         sqlc.arg(preview_access)::text
),
active_host AS MATERIALIZED (
  SELECT h.id
...
tpl AS MATERIALIZED (
  ...
  WHERE t.id = sqlc.arg(template_id)
    ...
    AND (t.team_id = sqlc.arg(team_id)
         OR t.team_id = sqlc.arg(system_team_id))
  FOR KEY SHARE OF t
), ins AS (
  ...
  SELECT sqlc.arg(id), sqlc.arg(team_id), sqlc.arg(name), sqlc.arg(status),
         ...
         sqlc.narg(snapshot_path), sqlc.narg(mem_path), sqlc.narg(base_path),
         sqlc.narg(delta_path), tpl.disk_mib,
         sqlc.narg(auto_delete_seconds)
  FROM tpl
  RETURNING *
), preview_policy AS (
  INSERT INTO sandbox_preview_policy (sandbox_id, access, revision)
...
WITH input_order AS MATERIALIZED (
  SELECT sqlc.arg(id)::uuid, sqlc.arg(team_id)::uuid, sqlc.arg(name)::text,
         ...
         sqlc.narg(base_path)::text, sqlc.narg(delta_path)::text,
         sqlc.narg(auto_delete_seconds)::int,
         sqlc.arg(preview_access)::text, sqlc.arg(secret_ids)::uuid[],
         sqlc.arg(env_keys)::text[], sqlc.arg(proxy_tokens)::text[]
),
active_host AS MATERIALIZED (
  SELECT h.id
  FROM host h
  WHERE h.id = @host_id AND h.status = 'active'
  FOR SHARE
),
admitted_host AS MATERIALIZED (
  SELECT id FROM active_host
  UNION ALL
  SELECT @host_id::text WHERE NOT EXISTS (SELECT 1 FROM host)
),
ins AS (
  INSERT INTO sandbox (id, team_id, name, status, vcpu_count, memory_mib, host_id, ip_address, pid, snapshot_id, timeout_seconds, metadata, template_id, snapshot_path, mem_path, base_path, delta_path, auto_delete_seconds)
  SELECT @id, @team_id, @name, @status, @vcpu_count, @memory_mib,
         admitted_host.id, @ip_address, @pid, @snapshot_id, @timeout_seconds,
         @metadata, @template_id, @snapshot_path, @mem_path, @base_path,
         @delta_path, @auto_delete_seconds
  FROM admitted_host
  RETURNING *
), preview_policy AS (
  INSERT INTO sandbox_preview_policy (sandbox_id, access, revision)
...
WITH input_order AS MATERIALIZED (
  -- Preserve this variant's pre-existing template/owner-first call order.
  SELECT sqlc.arg(template_id)::uuid, sqlc.arg(team_id)::uuid,
         sqlc.arg(system_team_id)::uuid, sqlc.arg(id)::uuid,
         sqlc.arg(name)::text, sqlc.arg(status)::sandbox_status,
         sqlc.arg(vcpu_count)::int, sqlc.arg(memory_mib)::int,
         sqlc.arg(host_id)::text, sqlc.narg(ip_address)::inet,
         sqlc.narg(pid)::int, sqlc.narg(snapshot_id)::uuid,
         sqlc.narg(timeout_seconds)::int, sqlc.arg(metadata)::jsonb,
         sqlc.narg(snapshot_path)::text, sqlc.narg(mem_path)::text,
         sqlc.narg(base_path)::text, sqlc.narg(delta_path)::text,
         sqlc.narg(auto_delete_seconds)::int,
         sqlc.arg(preview_access)::text, sqlc.arg(secret_ids)::uuid[],
...
tpl AS MATERIALIZED (
  ...
  INSERT INTO sandbox (id, team_id, name, status, vcpu_count, memory_mib, host_id, ip_address, pid, snapshot_id, timeout_seconds, metadata, template_id, snapshot_path, mem_path, base_path, delta_path, disk_mib, auto_delete_seconds)
  SELECT @id, @team_id, @name, @status, @vcpu_count, @memory_mib,
         tpl.host_id, @ip_address, @pid, @snapshot_id, @timeout_seconds,
         @metadata, tpl.tpl_id, @snapshot_path, @mem_path, @base_path,
         @delta_path, tpl.disk_mib, @auto_delete_seconds
  FROM tpl
  RETURNING *
), preview_policy AS (
  INSERT INTO sandbox_preview_policy (sandbox_id, access, revision)
