...
-- name: TryAcquireSavedSnapshotGlobalMigrationFence :one
...
-- name: TryAcquireSavedSnapshotTeamMigrationFence :one
...
SELECT pg_try_advisory_xact_lock_shared(
  hashtextextended(
    'superserve:migrate-team:' || sqlc.arg(team_id)::uuid::text,
    0
  )
)::boolean;
...
-- name: DeferSavedSnapshotReconciliation :execrows
...
WHERE id = sqlc.arg(snapshot_id)
  AND team_id = sqlc.arg(team_id)
  AND kind = 'saved'
  AND status = sqlc.arg(expected_status)::snapshot_status
  AND deleted_at IS NULL
