...
class WestPlanTests(unittest.TestCase):
    def verify(self, default, broken_identity=False, broken_endpoint=False, broken_status=False):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for role in ('primary', 'standby'):
                ...
                resources = {
                    ...
                    host + '.google_compute_instance.this': {'network_interface': [{'network_ip': ip}], 'tags': ['vmd-usw2'], 'desired_status': 'TERMINATED'},
...
if args[0] == 'plan':
    ...
    plan = json.loads(pathlib.Path(role + '.json').read_text())
    if '-var=primary_host_running=true' in args and os.environ['BROKEN_STATUS'] != '1':
        for resource in plan['planned_values']['root_module']['resources']:
            if resource['address'] == 'module.sandbox_host.google_compute_instance.this':
                resource['values']['desired_status'] = 'RUNNING'
    pathlib.Path(output).write_text(json.dumps(plan))
...
''')
            ...
            result = subprocess.run(['bash', str(root / 'scripts/terraform-plan-west.sh')],
                                    env=dict(os.environ, PATH=str(root) + os.pathsep + os.environ['PATH'], DEFAULT_ROLE=default, BROKEN_STATUS=str(int(broken_status))),
    ...
    def test_stopped_rollback_host_is_rejected(self):
        self.assertNotEqual(self.verify('standby', broken_status=True)[0], 0)
