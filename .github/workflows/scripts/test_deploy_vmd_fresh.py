        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            if configured:
                with (envdir / 'vmd.env').open('a') as env:
                    env.write('VMD_SCHEDULABLE_MEMORY_MIB=123456\nVMD_SCHEDULABLE_VCPUS=64\n')
...
'''
    ...
    def test_configured_hosts_preserve_omitted_inputs_and_reconcile_supplied_ones(self):
        for omitted in ('all', False):
            result, envs, modes = self.exercise(missing=(), configured=True, missing_input=omitted)
            self.assertEqual(modes, {"vmd.env": 0o600, "secretsproxy.env": 0o600})
            self.assertEqual(result.returncode, 0, result.stderr)
            ...
            self.assertIn('VMD_SCHEDULABLE_MEMORY_MIB=123456\n', envs['vmd.env'])
            self.assertIn('VMD_SCHEDULABLE_VCPUS=64\n', envs['vmd.env'])
    ...
    def test_blank_vmd_env_requires_explicit_kernel_selection(self):
        ...
        self.assertEqual(modes['vmd.env'], 0o600)
