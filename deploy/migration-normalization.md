...
| `e54a65ed` | fix: allow legacy proxy hosts before peer admission | #495 | `e6337694` | Durable.  |
| `91dcdc46` | test: cover staged peer credential admission | #495 | `e6337694` | Durable.  |
| `ebe161c9` | feat: select standby or serving hosts for manual deployments | #500 + #495 | `579c47d4 + e6337694` | Durable. Shared selector and VMD integration in #500; proxy integration in #495; no arbitrary labels. |
| `7fe9072b` | fix: reject use4 standby deployment until identity is provisioned | #500 | `579c47d4` | Durable.  |
| `001397ed` | fix: configure the use4 standby identity through deployment variables | #500 + #495 | `579c47d4 + e6337694` | Durable. Shared selector in #500, proxy workflow variable wiring in #495. |
...
| `1160deb9` | fix: recreate staging standby with creation-time workload identity | #500 | `579c47d4` | Mixed. Generic creation-time module, attestation flow and bootstrap retained; exact staging replacement/root/state changes excluded. |
| `52fea50b` | Add explicit peer credential providers and CAS staging issuance | #500 + #492 | `579c47d4 + 2c10f6ec` | Mixed. Provider validation/install retained in #500; plaintext regression in #492 at 2c10f6ec; staging issuance helper retained only on migration branch. |
...
| `f584e046` | Verify legacy proxy ingress with fresh control-plane heartbeat | #495 | `e6337694` | Durable.  |
...
Additional normalization regression: #492 `89598812` waits for asynchronous stale transport cleanup in the existing regression test.

No staging CI permission bypass, applied admission labels, VM replacement authorization, leaf certificate, host private key, or operator-specific IAM grant is imported. The opt-in creation-time host module is not wired into an existing VM resource by this normalization.

Validation: Linux deployment tests (91), peer bootstrap/provider (30), template storage (9), identity reconciliation (5), mocked creation-time host (4), mocked CAS custody (4), Terraform formatting. Full Go suite recorded after completion.
