...
type PreviewPort struct {
	...
	TokenVersion int64                  `protobuf:"varint,2,opt,name=token_version,json=tokenVersion,proto3" json:"token_version,omitempty"`
	// Per-port access mode: "public" routes without a token, "private" requires
	// this port's token. Empty (a sender that predates the field) falls back to
	// the sandbox-level preview_access; the proxy treats any other unknown value
...
const file_proto_vmd_proto_rawDesc = "" +
	...
	"\rtoken_version\x18\x02 \x01(\x03R\ftokenVersion\x12\x16\n" +
	"\x06access\x18\x03 \x01(\tR\x06access\"\xcc\x01\n" +
