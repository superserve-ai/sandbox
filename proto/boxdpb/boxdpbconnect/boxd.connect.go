...
const (
	...
	// DesktopServiceScreenshotProcedure is the fully-qualified name of the DesktopService's Screenshot
	// RPC.
	DesktopServiceScreenshotProcedure = "/superserve.boxd.v1.DesktopService/Screenshot"
...
type DesktopServiceClient interface {
	// Screenshot captures a single frame of the desktop. This is the primitive
	// for the agent control loop (screenshot -> decide -> act); unlike Stream it
	// holds no per-sandbox slot, so it always works even while a stream is
	// active.
	Screenshot(context.Context, *connect.Request[boxdpb.ScreenshotRequest]) (*connect.Response[boxdpb.ScreenshotResponse], error)
	...
	// SendActions executes an ordered batch of input actions in a single
	// request. The whole batch is validated before any action runs, then
	// executed under the same input lock that serializes individual input RPCs,
...
func NewDesktopServiceClient(httpClient connect.HTTPClient, baseURL string, opts ...connect.ClientOption) DesktopServiceClient {
	...
	return &desktopServiceClient{
		screenshot: connect.NewClient[boxdpb.ScreenshotRequest, boxdpb.ScreenshotResponse](
			httpClient,
			baseURL+DesktopServiceScreenshotProcedure,
			connect.WithSchema(desktopServiceMethods.ByName("Screenshot")),
			connect.WithClientOptions(opts...),
		),
		...
		sendActions: connect.NewClient[boxdpb.ActionBatch, boxdpb.ActionBatchResponse](
			httpClient,
			baseURL+DesktopServiceSendActionsProcedure,
...
// Screenshot calls superserve.boxd.v1.DesktopService.Screenshot.
func (c *desktopServiceClient) Screenshot(ctx context.Context, req *connect.Request[boxdpb.ScreenshotRequest]) (*connect.Response[boxdpb.ScreenshotResponse], error) {
...
func NewDesktopServiceHandler(svc DesktopServiceHandler, opts ...connect.HandlerOption) (string, http.Handler) {
	desktopServiceMethods := boxdpb.File_proto_boxd_proto.Services().ByName("DesktopService").Methods()
	desktopServiceScreenshotHandler := connect.NewUnaryHandler(
		DesktopServiceScreenshotProcedure,
		svc.Screenshot,
		connect.WithSchema(desktopServiceMethods.ByName("Screenshot")),
		connect.WithHandlerOptions(opts...),
	)
	...
	desktopServiceSendActionsHandler := connect.NewUnaryHandler(
		DesktopServiceSendActionsProcedure,
		svc.SendActions,
		connect.WithSchema(desktopServiceMethods.ByName("SendActions")),
		connect.WithHandlerOptions(opts...),
	)
	...
	return "/superserve.boxd.v1.DesktopService/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case DesktopServiceScreenshotProcedure:
			desktopServiceScreenshotHandler.ServeHTTP(w, r)
...
func (UnimplementedDesktopServiceHandler) Screenshot(context.Context, *connect.Request[boxdpb.ScreenshotRequest]) (*connect.Response[boxdpb.ScreenshotResponse], error) {
	return nil, connect.NewError(connect.CodeUnimplemented, errors.New("superserve.boxd.v1.DesktopService.Screenshot is not implemented"))
}
