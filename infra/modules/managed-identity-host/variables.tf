...
variable "managed_workload_identity" {
  description = "Creation-time managed identity, supplied only after trust-domain setup."
  type        = string
  validation {
    condition     = length(var.managed_workload_identity) > 0
    error_message = "A managed workload identity is required at creation."
  }
}
