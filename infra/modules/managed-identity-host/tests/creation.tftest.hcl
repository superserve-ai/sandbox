...
run "operator_admitted_host" {
  command = apply
  variables { labels = { component = "vmd", sandbox_status = "ready" } }
}

run "preserve_admission_during_maintenance_exclusion" {
  command = plan
  variables { labels = { component = "vmd-staging-standby" } }
  assert {
    condition     = google_compute_instance.this.labels.sandbox_status == "ready" && google_compute_instance.this.labels.component == "vmd-staging-standby"
    error_message = "Preserve operator admission while still allowing explicit deployment discovery changes."
  }
}
