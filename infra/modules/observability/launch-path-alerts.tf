...
locals {
  # Cloud Run documents the liveness-probe shutdown line in varlog/system.
  # Match that exact message rather than generic instance-start logs, which
  # also cover ordinary autoscaling and minimum-instance activity.
  cloud_run_churn_log_filter = <<-EOT
    log_id("varlog/system")
    resource.type="cloud_run_revision"
    resource.labels.service_name="%s"
    labels.instanceId:*
    textPayload =~ "^LIVENESS HTTP probe failed [0-9]+ times? consecutively for container .+ on port [0-9]+\\. The instance has been shut down\\.$"
    NOT jsonPayload.reason="DEPLOYMENT_ROLLOUT"
    NOT protoPayload.metadata.reason="DEPLOYMENT_ROLLOUT"
...
resource "google_monitoring_alert_policy" "cloud_run_churn" {
  ...
  documentation {
    content   = coalesce(each.value.documentation, "Unexpected Cloud Run replacement churn was detected for ${each.value.service_name}. The metric is limited to verified replacement events and excludes deployment rollouts.")
    mime_type = "text/markdown"
  }
