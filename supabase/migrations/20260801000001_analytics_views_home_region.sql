...
-- rows can still be present in the source cell's database (copied at
-- cutover) even though team.home_region now points at the destination
-- cell. Without this filter, a migrated team's pre-cutover usage counts
-- toward both cells' totals for the overlapping window — e.g.
-- jonathan@lindy.ai moved use -> usw; superserve-us-east's rank charts
-- kept ranking their pre-migration spend/sandbox counts even though that
-- team is no longer an east team.
