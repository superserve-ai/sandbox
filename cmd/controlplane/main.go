...
func run() error {
	...
	// restart or transient DB/VMD outage. V1 artifacts are host-local, so this
	// resolver deliberately has no default-host fallback.
...
func (c *grpcVMDClient) RestoreSavedSnapshot(ctx context.Context, vmID, manifestPath, manifestDigest, teamID, ownerID string, network vmdclient.SavedSnapshotNetwork, clearEnvKeys []string) (string, uint32, uint32, error) {
