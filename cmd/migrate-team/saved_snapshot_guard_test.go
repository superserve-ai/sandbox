...
func TestTeamMigrationFenceLockNameIsNamespacedAndPerTeam(t *testing.T) {
	teamA := uuid.MustParse("11111111-1111-1111-1111-111111111111")
	teamB := uuid.MustParse("22222222-2222-2222-2222-222222222222")

	got := teamMigrationFenceLockName(teamA)
	if want := "superserve:migrate-team:" + teamA.String(); got != want {
		t.Fatalf("lock name = %q, want %q", got, want)
	}
	if got == teamMigrationFenceLockName(teamB) {
		t.Fatal("different teams mapped to the same migration fence name")
	}
	if got == savedSnapshotGlobalMigrationFenceLockName {
		t.Fatal("team fence mapped to the global saved-snapshot fence name")
	}
}
